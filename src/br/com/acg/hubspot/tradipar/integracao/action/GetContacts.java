...
public class GetContacts implements ScheduledAction {
    ...
    public void onTime(ScheduledActionContext ctx) {
        ...
        try {
            ...
            // ====== CHAMADA HTTP ======
            String urlStr = Auth.HUBSPOT_BASE_URL + Auth.CONTACTS_ENDPOINT;
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                // ====== INSERT NA TGFCTT ======
                DynamicVO contatoVO = (DynamicVO) dwf.getDefaultValueObjectInstance("Contato");
                System.out.println("LOG-014");

                contatoVO.setProperty("CODPARC", codParcBD);  
                contatoVO.setProperty("CODCONTATO", codCtt); 
                contatoVO.setProperty("NOMECONTATO", firstname + " " + lastname);
                contatoVO.setProperty("EMAIL", email);
                
                PersistentLocalEntity inserted = dwf.createEntity("Contato", (EntityVO) contatoVO);
                DynamicVO novo = (DynamicVO) inserted.getValueObject();

                ctx.log("Contato inserido: CODPARC=" + codParc + " | CODCONTATO=" + novo.asBigDecimal("CODCONTATO"));
