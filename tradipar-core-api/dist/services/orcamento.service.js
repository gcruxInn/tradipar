...
exports.orcamentoService = exports.OrcamentoService = void 0;
...
class OrcamentoService {
     ...
     */
    async generateHeader(dealId) {
        console.log(`[OrcamentoService] Generating header for Deal ID: ${dealId}`);
        // 1. Fetch Deal Data
        const dealData = await hubspot_api_1.hubspotApi.getDeal(dealId, [
        ...
        const companyAssocResponse = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/deals/${dealId}/associations/companies`);
        ...
        const companyData = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/companies/${companyId}?properties=sankhya_codparc,codparc`);
        ...
        try {
            ...
            const parcResp = await sankhya_api_1.sankhyaApi.post('/gateway/v1/mge/service.sbr?serviceName=DbExplorerSP.executeQuery&outputType=json', parcQuery);
        ...
        const createResp = await sankhya_api_1.sankhyaApi.post(INCLUIR_NOTA_URL, notaBody);
     ...
     */
    async syncQuoteItems(dealId, nunota) {
        console.log(`[OrcamentoService] Syncing items for Deal ID: ${dealId} to NUNOTA: ${nunota}`);
        const errors = [];
        // 1. Fetch Line Items from HubSpot
        const lineItemsAssocResp = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/deals/${dealId}/associations/line_items`);
        ...
        for (const item of hubspotItems) {
            ...
            try {
                ...
                if (codLocalOrig === undefined || isNaN(codLocalOrig)) {
                    ...
                    if (!existing) {
                        const volResp = await sankhya_api_1.sankhyaApi.post('/gateway/v1/mge/service.sbr?serviceName=DbExplorerSP.executeQuery&outputType=json', {
                            requestBody: { sql: `SELECT CODVOL FROM TGFPRO WHERE CODPROD = ${item.codProd}` }
                        });
                        codVol = volResp.data.responseBody?.rows?.[0]?.[0] || 'UN';
                    }
                    const localResp = await sankhya_api_1.sankhyaApi.post('/gateway/v1/mge/service.sbr?serviceName=DbExplorerSP.executeQuery&outputType=json', {
        ...
        for (const [key, existing] of Object.entries(existingItems)) {
            if (!processedKeys.has(key)) {
                try {
                    ...
                    const delResp = await sankhya_api_1.sankhyaApi.post(`/gateway/v1/mgecom/service.sbr?serviceName=CACSP.excluirItemNota&outputType=json`, payload);
                    if (delResp.data.status !== "1")
                        throw new Error(delResp.data.statusMessage);
                }
                catch (delErr) {
    ...
    async getHubSpotItemsDetails(lineItemIds) {
        const items = [];
        const getErrors = [];
        if (lineItemIds.length === 0)
            return { items, getErrors };
        try {
            const lineItemsBatchResp = await hubspot_api_1.hubspotApi.post('/crm/v3/objects/line_items/batch/read', {
    ...
    async getSankhyaExistingItems(nunota) {
        const existingItems = {};
...
exports.orcamentoService = new OrcamentoService();
