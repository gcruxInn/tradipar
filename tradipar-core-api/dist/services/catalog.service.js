...
function parseDbExplorerFirstValue(data) {
    ...
    if (Array.isArray(rb?.rows) && rb.rows.length)
        return Number(rb.rows[0]?.[0]);
    const rsRows = rb?.resultSet?.rows;
    if (Array.isArray(rsRows) && rsRows.length)
        return Number(rsRows[0]?.PRECO?.$ || Object.values(rsRows[0])[0]?.$);
    return null;
}
class CatalogService {
     ...
     */
    async getStockAllUnits(codProd) {
        try {
            const sql = `
        SELECT 
          E.CODEMP,
     ...
     */
    async getStockMultipleProducts(codProds) {
        const results = [];
     ...
     */
    async getAllPrices(codProd, codParc, codEmp) {
     ...
     */
    async getDealSankhyaContext(objectId) {
        const deal = await hubspot_api_1.hubspotApi.getDeal(objectId, [
        ...
        let codParcRaw = null;
        // Fetch associations
        const assocResp = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/deals/${objectId}?associations=companies,contacts,quotes,line_items&properties=id`);
        const associations = assocResp.data.associations || {};
        const companyId = associations?.companies?.results?.[0]?.id;
        if (companyId) {
            try {
                const resp = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/companies/${companyId}?properties=sankhya_codparc,codparc`);
        ...
        if (!codParcRaw) {
            ...
            if (contactId) {
                try {
                    const resp = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/contacts/${contactId}?properties=sankhya_codparc,codparc`);
                    codParcRaw = resp.data.properties.sankhya_codparc || resp.data.properties.codparc;
                }
                catch (e) { }
            }
        ...
        if (lineItemIds.length > 0) {
            await Promise.all(lineItemIds.map(async (id) => {
                try {
                    const resp = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/line_items/${id}?properties=sankhya_codprod,codprod,hs_product_id,quantity,name,price,hs_sku,sankhya_controle,controle`);
                    const lp = resp.data.properties;
                    let codProd = lp.sankhya_codprod || lp.codprod || lp.hs_sku;
                    if (!codProd && lp.hs_product_id) {
                        const pResp = await hubspot_api_1.hubspotApi.get(`/crm/v3/objects/products/${lp.hs_product_id}?properties=sankhya_codprod,codprod,hs_sku`);
        ...
        if (quotes.length > 0) {
            try {
                const detailPromises = quotes.map((q) => hubspot_api_1.hubspotApi.get(`/crm/v3/objects/quotes/${q.id}?properties=hs_title,hs_status,hs_expiration_date,hs_lastmodifieddate`));
                ...
                activeQuote = details
                    ...
                    .sort((a, b) => new Date(b.properties.hs_lastmodifieddate).getTime() - new Date(a.properties.hs_lastmodifieddate).getTime())[0];
            }
            catch (e) {
        ...
        if (props.orcamento_sankhya && items.length > 0) {
            try {
                ...
                const queryResp = await sankhya_api_1.sankhyaApi.post('/gateway/v1/mge/service.sbr?serviceName=DbExplorerSP.executeQuery&outputType=json', {
                    serviceName: "DbExplorerSP.executeQuery",
                ...
                if (rowData && rowData.length > 0) {
                    ...
                    items.forEach((item) => {
                        let matchedIdx = sankhyaItems.findIndex((si) => si.codProd === item.codProd &&
        ...
        if (props.fase_negociacao)
            props.fase_negociacao = fixEncoding(props.fase_negociacao);
        if (props.dealstage)
            props.dealstage = fixEncoding(props.dealstage);
        return {
            objectId,
    ...
    async getDealPrices(objectId) {
        ...
        if (hasStockCut)
            console.warn(`[CORTE] Deal ${objectId} tem itens com falta de estoque.`);
        return {
            status: "SUCCESS",
    ...
    async searchProducts(query) {
        ...
        const searchResp = await hubspot_api_1.hubspotApi.post('/crm/v3/objects/products/search', searchBody);
        ...
        if (products.length > 0) {
          ...
          `;
                    ...
                    products.forEach((p) => {
                        const s = stocks.find((st) => String(st.CODPROD) === String(p.codProd));
                ...
                products.forEach((p) => {
                    if (p.stockMatriz === undefined)
                        p.stockMatriz = 0;
                    if (p.stockFilial === undefined)
                        p.stockFilial = 0;
                });
            }
        }
    ...
    // ================================================================
    async getProductControls(codProd, codEmp) {
