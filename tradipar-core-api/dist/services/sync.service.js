...
class SyncService {
    ...
    async executeSankhyaQuery(sql) {
        ...
        return rows.map((row) => {
            const obj = {};
            fieldNames.forEach((field, i) => { obj[field] = row[i]; });
            return obj;
        });
    }
    // Helper: find HubSpot Company by property
    async findHubSpotCompany(propertyName, value) {
        if (!value)
            return null;
        try {
            const response = await hubspot_api_1.hubspotApi.post('/crm/v3/objects/companies/search', {
    ...
    async findHubSpotProduct(sku) {
        if (!sku)
            return null;
        try {
            const response = await hubspot_api_1.hubspotApi.post('/crm/v3/objects/products/search', {
    ...
    `;
        ...
        for (const parc of partners) {
            ...
            let matchedBy = null;
            if (isPJ) {
                company = await this.findHubSpotCompany("cnpj", taxId);
                if (company)
                    matchedBy = "CNPJ";
            }
            if (!company && isPF) {
                company = await this.findHubSpotCompany("cpf", taxId);
                if (company)
                    matchedBy = "CPF";
            }
            if (!company) {
                company = await this.findHubSpotCompany("codparc", sankhyaId);
                if (company)
                    matchedBy = "CODPARC";
            }
            const properties = {
            ...
            if (isPJ)
                properties.cnpj = taxId;
            if (isPF)
                properties.cpf = taxId;
            Object.keys(properties).forEach(k => properties[k] === undefined && delete properties[k]);
            try {
                if (company) {
                    await hubspot_api_1.hubspotApi.patch(`/crm/v3/objects/companies/${company.id}`, { properties });
                    ...
                    const createResp = await hubspot_api_1.hubspotApi.post(`/crm/v3/objects/companies`, { properties });
    ...
    `;
        ...
        const mapIpiEntrada = (val) => { const s = val ? String(val).trim().padStart(2, '0') : ""; if (s === '49')
            return '49-Outras Entradas'; if (s === '03')
            return '03-Entrada Não Tributada'; if (s === '-1')
            return '(-1)-Não sujeita ao IPI'; return val ? String(val) : undefined; };
        const mapIpiSaida = (val) => { const s = val ? String(val).trim().padStart(2, '0') : ""; if (s === '99')
            return '99-Outras Saídas'; if (s === '53')
            return '53-Saída Não Tributada'; if (s === '-1')
            return '(-1)-Não sujeita ao IPI'; return val ? String(val) : undefined; };
        for (const prod of products) {
            ...
            try {
                if (hsProduct) {
                    await hubspot_api_1.hubspotApi.patch(`/crm/v3/objects/products/${hsProduct.id}`, { properties });
                    stats.updated++;
                    auditLog.push({ op: "UPDATE", sku, hubspotId: hsProduct.id });
                }
                else {
                    const createResp = await hubspot_api_1.hubspotApi.post(`/crm/v3/objects/products`, { properties });
                    stats.created++;
